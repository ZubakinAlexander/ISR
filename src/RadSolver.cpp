...
RadSolver::RadSolver(TGraphErrors* gr, double s_treshold) {
  ...
  std::sort(rightPart_.begin(), rightPart_.end(),
            [](const RightPart& x, const RightPart& y) { return x.s_ < y.s_; });
  double X[N];
  double Y[N];
  double EX[N];
  double EY[N];
...
TMatrixT<double> RadSolver::getEqMatrix() const {
  ...
  for (int n = 1; n <= N; ++n) {
    ...
    for (int i = 1; i <= n; ++i) {
      ...
      double xi = getX(n, i);
      auto linc = cscoeffs_linear(xm, xi, sn, xm, xi);
      A(n - 1, i - 1) += linc[1];
      if (i > 1) {
        A(n - 1, i - 2) += linc[0];
      }
...
TGraphErrors* RadSolver::getBornCS(
    TMatrixT<double>& invErrM, TMatrixT<double>& integralOperatorMatrix) const {
  ...
  TGraphErrors* gr =
      new TGraphErrors(N, ecm.GetMatrixArray(), cs.GetMatrixArray(),
                       ecm_err.GetMatrixArray(), cs_err.GetMatrixArray());
